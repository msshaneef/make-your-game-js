class Ghost {
  constructor(
    ...
    this.blinky = blinky;
    this.animationTarget = document.getElementById(name); //DOM element for animation

    this.reset(); //To get the ghosts' initial state
  }
  ...
  reset(fullGameReset) {
    if (fullGameReset) {
      ...
      delete this.cruiseElroy; //Specific state or behavior of the ghost
    }
  ...
  /**
   * This function sets the initial behavior of a ghost in a Pacman game.
   * It sets the ghost to scatter mode, allows collisions,
   * and sets its idle mode to 'idle' if the ghost is not 'blinky'.
   */
  setDefaultMode() {
  ...
  /**
   *
   * This code sets the movement properties of a ghost in a Pacman game.
   * It calculates the ghost's speeds (slow, medium, fast, scared, transition, and eye)
   * based on Pacman's speed and the game level.
   * It also sets the ghost's default direction based on its name (Blinky, Pinky, Inky, or Clyde).
   ...
   * This method is called to initialize the ghost's animation before the animation starts.
   */
  ...
  setStyleMeasurements(scaledTileSize, spriteFrames) {
    ...
    const bgSize = this.measurement * spriteFrames; //background size
...
class GameCoordinator {
  constructor() {
    ...
    this.restartButton.addEventListener(
      "click",
      this.restartButtonClick.bind(this)
    );
  ...
  restartButtonClick() {
    this.highScore = 0;
    this.pointsDisplay.textContent = "00";
    this.leftCover.style.left = "-50%";
    this.rightCover.style.right = "-50%";
    // this.leftCover.style.left = "-50%";
    // this.rightCover.style.right = "-50%";
